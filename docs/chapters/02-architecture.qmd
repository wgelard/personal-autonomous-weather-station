...
This requires the mode switch to be wired to an EXT1-capable GPIO and registered as a wakeup source, which is what disambiguates a switch-flip from a fresh power-on. States are **mutually exclusive**. The FSM is deterministic and fully observable via the status LED.

| State | LED |
|-------|-----|
| `BOOT` / `CHECK_WAKEUP` / `READ_SWITCH` / `DATA_LOGGER` | Off |
| `INIT_RTC` — NTP sync in progress | Blinking |
| `SERVER` | Solid ON |
| `DEEP_SLEEP` | Off |
