...
flowchart LR
    ...
    subgraph Backend["🖥️ Odroid C4 Server"]
        API[REST API\nFastAPI]
        DB[Database\nTinyDB / InfluxDB]
        DASH[Dashboard\nGrafana]
        ML[ML Models\nRF + LSTM]
...
stateDiagram-v2
    ...
    BOOT --> INIT_RTC : RTC not valid
    BOOT --> DATA_LOGGER : Normal wake (RTC alarm)

    SERVER --> INIT_RTC : Long button press
    INIT_RTC --> SERVER : NTP sync successful

    DATA_LOGGER --> DEEP_SLEEP : Measurements written
    SERVER --> DEEP_SLEEP : Switch toggled back to MEASURE
    INIT_RTC --> DEEP_SLEEP : (blocked until RTC valid)

    DEEP_SLEEP --> [*]
```

States are **mutually exclusive**. The FSM is deterministic and fully observable via the status LED.
...
3. User (or script) downloads the latest CSV log file(s)
4. When back on home Wi-Fi, a script pushes the data to the Odroid C4 server

This approach requires **zero infrastructure at the station location**. The station can be in a garden, a field, or a rooftop — anywhere.

A simple Python script on the phone (via Termux on Android, or a laptop) is sufficient for the gateway role.

---

## Backend — server role

The Odroid C4 is the home server. It is always on (or scheduled) and provides:
...
- **Visualization** — Grafana dashboard with historical data and derived metrics
