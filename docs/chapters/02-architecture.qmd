...
stateDiagram-v2
    ...
    CHECK_WAKEUP --> SERVER : GPIO mode switch = SERVER
    CHECK_WAKEUP --> INIT_RTC : Power-on / reset (or RTC invalid)
    CHECK_WAKEUP --> DATA_LOGGER : RTC alarm interrupt
...
At every boot, `esp_sleep_get_wakeup_cause()` is called first to read the hardware wakeup reason (power-on reset, RTC alarm interrupt, or GPIO change), then the mode switch GPIO is sampled to determine the target state. States are **mutually exclusive**. The FSM is deterministic and fully observable via the status LED.
